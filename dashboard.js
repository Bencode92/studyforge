...
// Reads existing quiz.history[] and quiz.weakPoints[] to prioritize fiches
// No new data needed - uses 100% of existing fiche JSON structure

// === REVIEW PRIORITY CALCULATOR ===
function calculateReviewPriority(ficheData) {
  ...
  // Boost priority by weakPoints count
...
// === DASHBOARD VIEW ===
// Override render to add dashboard when no fiche is selected
...
render = function() {
  ...
  // Only show dashboard on home page with no category selected
  if (currentView === 'home' && !selCat && !fiche) {
    // Check if we have any categories with fiches
...
async function loadDashboard(container) {
  // Collect all fiches from all categories
  const allFiches = [];

  for (const cat of cats) {
    try {
      const meta = await ghGet('data/' + cat.id + '/_meta.json');
      const ficheIds = meta?.content?.fiches || [];
      
      // If no fiches in meta, try listing directory
      let ids = ficheIds;
      if (ids.length === 0) {
        const files = await ghLs('data/' + cat.id);
        ids = files
          .filter(f => f.name.endsWith('.json') && f.name !== '_meta.json')
      ...
      for (const f of ids) {
        try {
          ...
          if (ficheData?.content) {
            ...
            allFiches.push({
              id: f.id,
              title: ficheData.content.metadata?.title || f.title || f.id,
              category: cat,
              review,
              sections: ficheData.content.base?.sections?.length || 0,
              enrichments: ficheData.content.enrichments?.length || 0,
              version: ficheData.content.metadata?.version || 1
            });
          }
        } catch (e) { /* skip broken fiches */ }
      }
    } catch (e) { /* skip broken categories */ }
  }
  ...
  h += '<h2 style="font-size:24px;font-weight:800;margin-bottom:4px">StudyForge</h2>';
  h += '<p style="color:#6b6b88;font-size:13px">' + allFiches.length + ' fiches · ' + cats.length + ' catégories</p>';
  h += '</div>';
  ...
  // Fiche list grouped by priority
  const groups = [
  ...
  groups.forEach(g => {
    ...
    h += '<h3 style="font-size:15px;font-weight:700;margin-bottom:4px">' + g.title + '</h3>';
    h += '<p style="font-size:11px;color:#6b6b88;margin-bottom:12px">' + g.desc + '</p>';

    fiches.forEach(f => {
      h += '<div class="card" style="padding:14px 18px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:14px;border-left:3px solid ' + f.review.color + '" ' +
        'onclick="loadCat(\'' + f.category.id + '\').then(()=>loadFiche(\'' + f.id + '\'))">';

      // Left: emoji + info
      h += '<div style="flex:1;min-width:0">';
      h += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:3px">';
      h += '<span style="font-size:14px">' + (f.category.icon || '📁') + '</span>';
      h += '<span style="font-size:14px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(f.title) + '</span>';
      h += '</div>';
      h += '<div style="display:flex;gap:8px;align-items:center;font-size:10px;color:#6b6b88">';
      h += '<span>' + f.category.name + '</span>';
      h += '<span>·</span>';
      h += '<span>' + f.sections + ' sections</span>';
      ...
      if (f.review.weakCount > 0) {
        h += '<span class="tag tag-org" style="font-size:9px">' + f.review.weakCount + ' pts faibles</span>';
      }
      if (f.review.score !== null) {
        const scoreColor = f.review.score >= 80 ? '#34d399' : f.review.score >= 60 ? '#fbbf24' : '#f87171';
        h += '<div style="text-align:center"><div style="font-size:18px;font-weight:800;color:' + scoreColor + '">' + f.review.score + '%</div><div style="font-size:9px;color:#6b6b88">' + f.review.attempts + ' quiz</div></div>';
      }
      if (f.review.daysSince !== null) {
        h += '<div style="text-align:center;min-width:40px"><div style="font-size:12px;font-weight:600;color:#9999b0">' + f.review.daysSince + 'j</div><div style="font-size:9px;color:#6b6b88">depuis</div></div>';
      }
      h += '</div>';
  ...
  if (allFiches.length === 0) {
    h += '<div style="text-align:center;padding:40px;color:#6b6b88">';
    h += '<p style="font-size:14px">Aucune fiche pour le moment</p>';
    h += '<p style="font-size:12px;margin-top:8px">Clique sur "+ Fiche" pour importer ton premier document</p>';
    h += '</div>';
  }
...
console.log('v17: Dashboard "À réviser" loaded');
